...
#define DELETED_ADDRESS (person *)(0xFFFFFFFFF)
...
void print_table() {
  ...
  for (int i = 0; i < TABLE_SIZE; i++) {
    if (hash_table[i] == NULL) {
      printf("\t%i\t---\n", i);
    } else if (hash_table[i] == DELETED_ADDRESS) {
      printf("\t--<deleted>\n");
    } else {
      printf("\t%i\t%s\n", i, hash_table[i]->name);
    }
...
person *hash_table_lookup(char *name) {
  ...
  for (int i = 0; i < TABLE_SIZE; i++) {
    ...
    if (hash_table[try] == DELETED_ADDRESS) {
      continue;
    }
...
bool hash_table_insert(person *p) {
  ...
  for (int i = 0; i < TABLE_SIZE; i++) {
    ...
    if (hash_table[try] == NULL || hash_table[try] == DELETED_ADDRESS) {
      hash_table[try] = p;
      return true;
    }
  }
...
person *hash_table_delete(char *name) {
  ...
  for (int i = 0; i < TABLE_SIZE; i++) {
    ...
    if (strncmp(hash_table[index]->name, name, TABLE_SIZE) == 0) {
      ...
      hash_table[try] = DELETED_ADDRESS;
      return found;
    }
  }
  return NULL;
